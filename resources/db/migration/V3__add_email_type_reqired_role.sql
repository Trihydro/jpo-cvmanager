...
-- Adds a required_role column to email_type, linking each notification
-- email category to the minimum role a user must hold (within their organization)
-- to receive that email type.
--
-- Steps performed:
--   1. Add required_role (integer, nullable) with a FK to roles(role_id).
...
-- Update email_type table definition
