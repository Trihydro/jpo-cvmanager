...
DROP INDEX IF EXISTS idx_intersection_id;

-- Redundant with intersection_intersection_number UNIQUE constraint
DROP INDEX IF EXISTS idx_intersections_intersection_number;

-- Redundant with rsu_ipv4_address UNIQUE constraint
DROP INDEX IF EXISTS idx_rsus_ipv4_address;
...
DROP INDEX IF EXISTS idx_rsus_ipv4_rsu_id;
...
DROP INDEX IF EXISTS idx_scms_health_timestamp;
...
CREATE INDEX idx_ping_rsu_id_timestamp
    ON ping (rsu_id, timestamp DESC);

COMMENT ON INDEX idx_ping_rsu_id_timestamp IS
...
CREATE INDEX idx_rsu_health_rsu_id_timestamp
    ON rsu_health (rsu_id, timestamp DESC);

COMMENT ON INDEX idx_rsu_health_rsu_id_timestamp IS
...
CREATE INDEX idx_scms_health_rsu_id_timestamp
    ON scms_health (rsu_id, timestamp DESC);

COMMENT ON INDEX idx_scms_health_rsu_id_timestamp IS
    'Replaces the dropped idx_scms_health_timestamp (single-column, unused). '
    'The leading rsu_id column supports per-RSU certificate health lookups. '
    'The trailing timestamp DESC column allows PostgreSQL to satisfy '
...
COMMENT ON INDEX idx_user_organization_organization_id IS
...
CREATE INDEX idx_user_email_notification_email_type_id
    ON user_email_notification (email_type_id);

COMMENT ON INDEX idx_user_email_notification_email_type_id IS
...
CREATE INDEX idx_firmware_upgrade_rules_from_id
    ON firmware_upgrade_rules (from_id);

COMMENT ON INDEX idx_firmware_upgrade_rules_from_id IS
...
CREATE INDEX idx_rsu_options_tim_deposit
    ON rsu_options (rsu_id)
    WHERE tim_deposit = true;

COMMENT ON INDEX idx_rsu_options_tim_deposit IS
    'Partial index covering only rows where tim_deposit = true. '
...
CREATE INDEX idx_rsus_primary_route
    ON rsus (primary_route);

COMMENT ON INDEX idx_rsus_primary_route IS
...
CREATE INDEX idx_rsu_intersection_intersection_id
    ON rsu_intersection (intersection_id);

COMMENT ON INDEX idx_rsu_intersection_intersection_id IS
