...
def get_rsu_upgrade_data(rsu_ip="all"):
    query = (
        ...
        "JOIN cvmanager.rsu_models rm ON rm.rsu_model_id = rd.model "
        "JOIN cvmanager.manufacturers man ON man.manufacturer_id = rm.manufacturer "
        "JOIN cvmanager.rsu_credentials rc ON rc.credential_id = rd.credential_id "
        "JOIN cvmanager.firmware_upgrade_rules fur ON fur.from_id = rd.firmware_version "
        "JOIN cvmanager.firmware_images fi ON fi.firmware_id = rd.target_firmware_version "
    ...
    with active_upgrades_lock:
        ...
        if request_args["status"] == "success":
            ...
            try:
                ...
                query = f"UPDATE cvmanager.rsus SET firmware_version={upgrade_info['target_firmware_id']} WHERE ipv4_address='{request_args['rsu_ip']}'"
            ...
            if is_rsu_at_max_retries_limit(request_args["rsu_ip"]):
                ...
                query = f"UPDATE cvmanager.rsus SET target_firmware_version=firmware_version WHERE ipv4_address='{request_args['rsu_ip']}'"
                pgquery.write_db(query)
