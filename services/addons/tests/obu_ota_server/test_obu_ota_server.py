...
async def test_log_request(mock_removed_old_logs, mock_datetime, mock_pgquery):
    ...
    expected_query = (
        f"INSERT INTO obu_ota_requests (obu_sn, manufacturer, request_datetime, origin_ip, obu_firmware_version, requested_firmware_version, error_status, error_message) VALUES"
...
def test_removed_old_logs_with_removal(mock_pgquery):
    ...
    mock_pgquery.query_db.assert_any_call(
        f"SELECT request_id FROM obu_ota_requests WHERE obu_sn = '{serialnum}' AND error_status = B'0' ORDER BY request_datetime ASC LIMIT 5"
    )
    mock_pgquery.write_db.assert_called_once_with(
        "DELETE FROM obu_ota_requests WHERE request_id IN (1,2,3,4,5)"
    )
