...
def get_org_data(org_name: str, is_admin_in_org: bool):
    ...
    rsu_query = (
        ...
        "LEFT JOIN cvmanager.rsu_options opts ON rsus.rsu_id = opts.rsu_id"
...
def modify_org_authorized(orig_name: str, org_spec: dict, is_bulk_update: bool = False):
    ...
    try:
        ...
        if is_bulk_update and (
            ...
            bulk_update_query = (
                f"INSERT INTO cvmanager.rsu_options ({', '.join(insert_columns)}) "
                f"SELECT {', '.join(select_columns)} "
                "FROM cvmanager.rsu_organization ro "
                "JOIN cvmanager.organizations org ON ro.organization_id = org.organization_id "
                "WHERE org.name = :name "
