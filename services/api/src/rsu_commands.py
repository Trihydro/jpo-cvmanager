...
def fetch_rsu_info(rsu_ip, organization):
    ...
    query = (
        ...
        "FROM rsus AS rd "
        "JOIN rsu_organization_name AS ron_v ON ron_v.rsu_id = rd.rsu_id "
        "JOIN rsu_models AS rm ON rm.rsu_model_id = rd.model "
        "JOIN manufacturers AS man ON man.manufacturer_id = rm.manufacturer "
        "LEFT JOIN rsu_credentials AS rsu_creds ON rsu_creds.credential_id = rd.credential_id "
        "LEFT JOIN snmp_credentials AS snmp ON snmp.snmp_credential_id = rd.snmp_credential_id "
        "LEFT JOIN snmp_protocols AS snmp_ver ON snmp_ver.snmp_protocol_id = rd.snmp_protocol_id "
