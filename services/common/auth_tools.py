...
def get_rsu_set_for_org(organizations: list[str]) -> set[str]:
    ...
    query = (
        ...
        "FROM rsus rsu "
        "JOIN rsu_organization AS rsu_org ON rsu_org.rsu_id = rsu.rsu_id "
...
def check_intersection_with_org(intersection_id: str, organizations: list[str]) -> bool:
    ...
    query = (
        ...
        "FROM intersections intersection "
        "JOIN intersection_organization AS intersection_org ON intersection_org.intersection_id = intersection.intersection_id "
        "JOIN organizations AS org ON org.organization_id = intersection_org.organization_id "
        f"WHERE org.name IN ({allowed_orgs_placeholder}) "
...
def check_user_with_org(user_email: str, organizations: list[str]) -> bool:
    ...
    query = (
        ...
        "FROM users u "
        "JOIN user_organization AS user_org ON user_org.user_id = u.user_id "
        "JOIN organizations AS org ON org.organization_id = user_org.organization_id "
        f"WHERE org.name IN ({allowed_orgs_placeholder}) "
...
def get_user_info(email: str) -> Optional[UserInfo]:
    ...
    org_query = (
        ...
        "JOIN organizations org on uo.organization_id = org.organization_id "
...
def get_qualified_org_list(
    ...
    if include_super_user and user.user_info.super_user:
        return pgquery.query_and_return_list(
            "SELECT name FROM organizations ORDER BY name ASC"
        )
    allowed_orgs = []
